...
public class CERP005_SearchQuestionUser
{
	...
	private final static String title_text = "Nadawanie uprawnie do edycj jaźń ąęśćń i 123450 ŁÓDŹ dokumentłw w innym cen łódź PÓjść";
	private final static String contents_text = "edytowanie ąęśćń dokumentw z innego 12345 ŁÓDŹ centrum nidomylnie ustawione.Choddzi o jażń to iytkownik pracuje na wicej jednym centrum ";
	private final static String answer_contents_text = "qasw edytowanie dokumentw łódźz ŁÓDŹ innego 12345 jażń centrum ąęśćń nidomylnie ustawione.Choddzi pójść o to iytkownik pracuje n jaźń ŁóDŹ";
	...
	public void getDriver(ITestContext context)
	{
		...
		// FF
		if (version.equalsIgnoreCase("30"))
			capabilities.setVersion(version);
	...
	public void TestMethod() throws Exception
	{
		try
		{
			...
			CERP_SearchPage searchPage = mainPage.clickSearch();
			searchPage.searchTitle(title_text);
			searchPage.searchContents(contents_text);
			searchPage.searchAnswerContents(answer_contents_text);
			CERP_QuestionPage questionPage1 = searchPage.clickQuestion();
			questionPage1.deleteQuestion();
			questionPage1.searchDeleteQuestion(title_text, message_text);
			CERP_SearchPage searchPage1 = questionPage1.clickSearch();
			searchPage.searchDeleteQuestion(title_text);
			searchPage.searchDeleteAnswer(answer_contents_text);
			searchPage1.searchUser(user_name);
